...
class DINOImage:
    ...
    def preprocess_frames(self, frames):
        frames = frames.float() / 255.0 # normalize
...
def compute_dino_image(video_list, device, submodules_list, **kwargs):
    ...
    dino_image = DINOImage(device)
    all_results, video_results = dino_image.compute_similarity(video_dict, n_frames=n_frames, mode=mode)
    if get_world_size() > 1:
