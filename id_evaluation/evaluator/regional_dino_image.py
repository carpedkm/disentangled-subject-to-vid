...
class RegionalDINOImage:
    def __init__(self, device):
        self.device = device
        self.load_model()
    
    def load_model(self): #TODO: maybe need to change tsfm to resize_and_padding instead of resize_and_crop
        self.model = torch.hub.load('facebookresearch/dino:main', 'dino_vits16').to(self.device)
    ...
    def preprocess_frames(self, frames):
        frames = [self.transform_pil(frame) for frame in frames]
    ...
    def compute_similarity(self, video_dict, n_frames='full', mode='default'):
        ...
        for video_path, image_path in tqdm(video_dict.items()):
            ...
            image = self.transform_pil(image).unsqueeze(0).to(self.device)
            with torch.no_grad():
...
def compute_regional_dino_image(video_list, device, submodules_list, **kwargs):
    if isinstance(video_list, list):
        raise TypeError("video_list should be a dictionary for dino image")
    ...
    regional_dino_image = RegionalDINOImage(device)
    all_results, video_results = regional_dino_image.compute_similarity(video_dict, n_frames=n_frames, mode=mode)
    if get_world_size() > 1:
