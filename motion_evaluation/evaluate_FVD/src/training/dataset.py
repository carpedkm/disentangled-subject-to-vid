...
class VideoFramesFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,                                 # Additional arguments for the Dataset base class.
        ...
        listdir_full_paths = lambda d: sorted([os.path.join(d, x) for x in os.listdir(d) if not x.split('.')[-1]=='db'])
        name = os.path.splitext(os.path.basename(self._path))[0]
        ...
        # if 'data_path_additional' in cfg:
        #     self._additional_path = cfg.data_path_additional
        #     self._additional_objects = {o for d in listdir_full_paths(self._additional_path) for o in (([d] + listdir_full_paths(d)) if os.path.isdir(d) else [d])}
        #     self._additional_objects = {os.path.relpath(o, start=os.path.dirname(self._additional_path)) for o in {self._additional_path}.union(self._additional_objects)}
        #     pdb.set_trace()
        #     tmp = self._all_objects.union(self._additional_objects)
