...
class ImageFolderDataset(Dataset):
    ...
    def _load_raw_image(self, raw_idx):
        ...
        with self._open_file(fname) as f:
            ...
            image = load_image_from_buffer_resize(f, resolution=self._resolution, use_pyspng=use_pyspng)
...
class VideoFramesFolderDataset(Dataset):
    def __init__(self,
        path_list,                                           # Path to directory or zip.
        ...
        **super_kwargs,                                 # Additional arguments for the Dataset base class.
        ...
        for _path in path_list:
            ...
            if os.path.isdir(_path):
                ...
                _all_objects = {o for d in listdir_full_paths(_path) for o in (([d] + listdir_full_paths(d)) if os.path.isdir(d) else [d])}
                _all_objects = {os.path.relpath(o, start=os.path.dirname(_path)) for o in {_path}.union(_all_objects)}
            else:
                raise IOError('Path must be either a directory or point to a zip archive')
            
            PIL.Image.init()
            _video_dir2frames = {}
            objects = sorted([d for d in _all_objects])
            root_path_depth = len(os.path.normpath(objects[0]).split(os.path.sep))
            curr_d = objects[1] # Root path is the first element
            
            _root_path = os.path.dirname(_path)
            
            for o in objects[1:]:
                curr_obj_depth = len(os.path.normpath(o).split(os.path.sep))
                # print(curr_obj_depth)
                ...
                if self._file_ext(o) in PIL.Image.EXTENSION:
                    ...
                    if not curr_d in _video_dir2frames:
                        _video_dir2frames[curr_d] = []
                    _video_dir2frames[curr_d].append(os.path.join(_root_path , o))
                    ...
                    if curr_d in _video_dir2frames:
                        sorted_files = sorted(_video_dir2frames[curr_d])
                        sorted_files = [os.path.join(_root_path , x) for x in sorted_files]
                        _video_dir2frames[curr_d] = sorted_files
                    curr_d = o

            _video_idx2frames = [frames for frames in _video_dir2frames.values()]
            
            
            # Remove Spin in the generated videos. (Definitely.. there are no videos with spin camera trajectory.)
            new_list = []
            for video in _video_idx2frames:
                if 'SPIN' in video[0].split('/')[-2]:
                    continue
                new_list.append(video)
            _video_idx2frames = new_list
            
            total_path_list.extend(_video_idx2frames)

        
        self._video_idx2frames = total_path_list


        if 'exclude_path' in cfg:
            import json
            with open(cfg['exclude_path'], 'r') as f:
                exclude_list = json.load(f)
            exclude_list = [str(x) for x in exclude_list]
            self.exclude_list = exclude_list
            
            
            new_list = []
            for video in self._video_idx2frames:
                if video[0].split('/')[-2] in self.exclude_list or video[0].split('/')[-2].split('_')[0] in self.exclude_list:
                    continue
                new_list.append(video)
            self._video_idx2frames = new_list
...
def load_image_from_buffer_resize(f, resolution, use_pyspng: bool=False) -> np.ndarray:
    if use_pyspng:
        image = pyspng.load(f.read())
    else:
        # resize the image to the desired resolution
        timg = PIL.Image.open(f)
        w, h = timg.size
        if w==resolution:
            image = np.array(timg)
        else:
            image = np.array(timg.resize((resolution, resolution), PIL.Image.LANCZOS))
        # del timg
        del timg
    if image.ndim == 2:
        image = image[:, :, np.newaxis] # HW => HWC
