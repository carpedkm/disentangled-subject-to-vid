...
class VideoDataset(Dataset):
    ...
    def _process_single_video(self, video_path, train_transforms):
        try:
            import decord
        except ImportError:
            raise ImportError(
        ...
        decord.bridge.set_bridge("native")  # Set bridge to native
        video_reader = decord.VideoReader(uri=Path(video_path).as_posix(), width=self.width, height=self.height)
...
def main(args):
    ...
    # # For CLIPTextProjectionLayer
    # transformer.CLIPTextProjectionLayer = ProjectionLayer(in_features=512, out_features=4096)
    # init.xavier_uniform_(transformer.CLIPTextProjectionLayer.projection.weight)
    # if transformer.CLIPTextProjectionLayer.projection.bias is not None:
    #     init.zeros_(transformer.CLIPTextProjectionLayer.projection.bias)

    # # For CLIPVisionProjectionLayer
    # transformer.CLIPVisionProjectionLayer = ProjectionLayer(in_features=768, out_features=4096)
    # init.xavier_uniform_(transformer.CLIPVisionProjectionLayer.projection.weight)
    # if transformer.CLIPVisionProjectionLayer.projection.bias is not None:
    #     init.zeros_(transformer.CLIPVisionProjectionLayer.projection.bias)       
    transformer.CLIPTextProjectionLayer = ProjectionLayer(in_features=512, out_features=4096)
    with torch.no_grad():
        ...
        transformer.CLIPTextProjectionLayer.projection2.weight.fill_(0.0)
        if transformer.CLIPTextProjectionLayer.projection2.bias is not None:
            transformer.CLIPTextProjectionLayer.projection2.bias.fill_(0.0)
    transformer.CLIPVisionProjectionLayer = ProjectionLayer(in_features=768, out_features=4096)
    with torch.no_grad():
        ...
        transformer.CLIPVisionProjectionLayer.projection2.weight.fill_(0.0)
        if transformer.CLIPVisionProjectionLayer.projection2.bias is not None:
            transformer.CLIPVisionProjectionLayer.projection2.bias.fill_(0.0)
    # Requires grad true
