    ...
    def __init__(
        ...
        if self.load_to_ram is True:
            ...
            if self.multi_frames is True:
                ...
                self.instance_right_multi_latent_root_map_with_id = {}
                for id in tqdm(self.ids):
                    self.instance_left_multi_latent_root_map_with_id[id] = np.load(os.path.join(self.instance_left_multi_latent_root, f'left_{id}_vae_latents.npy'))
                    self.instance_right_multi_latent_root_map_with_id[id] = np.load(os.path.join(self.instance_right_multi_latent_root, f'right_{id}_vae_latents.npy'))
                    ...
                    if target == 0: # left : condition / right : target
                        ...
                        if not self.load_to_ram:
                            if not self.multi_frames:
                                ...
                                latent = torch.from_numpy(np.load(self.instance_right_multi_latent_root_map_with_id[index]))
                        else:
                            if not self.multi_frames:
                                latent = torch.from_numpy(self.instance_right_latent_root_map_with_id[index])
                            else:
                                latent = torch.from_numpy(self.instance_right_multi_latent_root_map_with_id[index])
                        ...
                        if not self.load_to_ram:
                            if not self.multi_frames:
                                ...
                                latent = torch.from_numpy(np.load(self.instance_left_multi_latent_root_map_with_id[index]))
                        else:
                            if not self.multi_frames:
                                latent = torch.from_numpy(self.instance_left_latent_root_map_with_id[index])
                            else:
                                latent = torch.from_numpy(self.instance_left_multi_latent_root_map_with_id[index])
