...
def get_args():
    ...
    parser.add_argument(
        '--add_noise_mix',
        action='store_true',
        help='Whether to use add noise mix or not'
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # if args.add_noise_mix: # following standard I2V pipeline
                #     image_noise_sigma = torch.normal(mean=-3.0, std=0.5, size=(1,), device=accelerator.device)
                #     image_noise_sigma = torch.exp(image_noise_sigma).to(dtype=weight_dtype)
                ...
                model_output = transformer(
                    ...
                    # add_noise_mix=args.add_noise_mix,
                )[0]
